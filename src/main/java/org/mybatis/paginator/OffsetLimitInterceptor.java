...
import org.mybatis.paginator.dialect.Dialect;
import org.mybatis.paginator.model.PageBounds;
import org.mybatis.paginator.model.PageList;
import org.mybatis.paginator.model.Paginator;
import org.mybatis.paginator.support.PropertiesUtils;
import org.mybatis.paginator.support.SQLUtils;
...
public class OffsetLimitInterceptor implements Interceptor {
    ...
    public Object intercept(final Invocation invocation) throws Throwable {
        ...
        if (pageBounds.getOffset() == RowBounds.NO_ROW_OFFSET
            ...
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug("Skip to run OffsetLimitInterceptor");
            }
            return invocation.proceed();
        }

        if(RowBounds.class.equals(queryArgs[ROWBOUNDS_INDEX].getClass())) {
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug("Use RowBounds, skip to run OffsetLimitInterceptor");
            }
            return invocation.proceed();
        }
            ...
            Callable<Paginator> countTask = new Callable() {
                public Object call() throws Exception {
                    ...
                    if (cache != null && ms.isUseCache() && ms.getConfiguration().isCacheEnabled()) {
                        ...
                        if (count == null) {
                            count = SQLUtils.getCount(ms, parameter, boundSql, dialect);
    ...
    public void setProperties(Properties properties) {
        ...
        String dialectClass = propertiesUtils.getRequiredString("dialectClass");
        setDialectClass(dialectClass);

        setAsyncTotalCount(propertiesUtils.getBoolean("asyncTotalCount", false));

        setPoolMaxSize(propertiesUtils.getInt("poolMaxSize", 0));
